...
#Parameters and functions for EadyUnstable script. In the frame of the wave (c.real=0).
...
def phiprime_x(x,z,t):
    ...
    #return (k*L*(np.cos(k*L*x)*((np.cosh(z*Hratio))/(np.cosh(Hratio))) - np.sin(k*L*x)*((np.sinh(z*Hratio))/(np.sinh(Hratio))))*np.exp(sigma_max*T*t))
...
def phiprime_z(x,z,t):
    ...
    #return (Hratio*(np.cos(k*L*x)*((np.cosh(z*Hratio))/(np.sinh(Hratio))) + np.sin(k*L*x)*((np.sinh(z*Hratio))/(np.cosh(Hratio))))*np.exp(sigma_max*T*t))
...
def phiprime_t(x,z,t):
    ...
    #return (sigma_max*T*(np.cos(k*L*x)*((np.sinh(z*Hratio))/(np.sinh(Hratio))) + np.sin(k*L*x)*((np.cosh(z*Hratio))/(np.cosh(Hratio))))*np.exp(sigma_max*T*t))
...
def phiprime_zx(x,z,t):
    ...
    #return (k*L*Hratio*(np.cos(k*L*x)*((np.sinh(z*Hratio))/(np.cosh(Hratio))) - np.sin(k*L*x)*((np.cosh(z*Hratio))/(np.sinh(Hratio))))*np.exp(sigma_max*T*t))
...
def phiprime_zt(x,z,t):
    ...
    #return (sigma_max*T*(Hratio*(np.cos(k*L*x)*((np.cosh(z*Hratio))/(np.sinh(Hratio))) + np.sin(k*L*x)*((np.sinh(z*Hratio))/(np.cosh(Hratio))))*np.exp(sigma_max*T*t)))

def vprime(x,z,t): #Non-dimensional meridional velocity perturbation, Gill 12.9.3
    return (1./(L*(f0**2)))*phiprime_x(x,z,t)
    #return (1./((L**2)*f0))*phiprime_x(x,z,t)
